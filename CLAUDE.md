...
- デプロイ(`main`へのgit push)はClaudeが行う。`main`はGitHub Pagesから直接配信されるため、pushした時点でそのまま本番に反映される。テストスイートが存在せずビルドの正しさを機械的に保証できないため、push前に必ず`bash build.sh`を実行して`jintoria.html`を最新化し、`node --check`(利用可能な場合)でJS構文を確認すること。確認が取れたら、ユーザーへの都度の承認を待たずに`main`へpushしてよい。
